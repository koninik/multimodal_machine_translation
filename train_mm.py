...
import transformer_mmt.Constants as Constants
from dataset_mm import TranslationDataset, paired_collate_fn
from transformer_mmt.Models import Transformer
from transformer_mmt.Optim import ScheduledOptim
        ...
        if args.save_model:
            if args.save_mode == 'all':
                ...
                if num_of_no_improvement >= STOPPING_CRITERIA_EPOCHS:
                    with open("validation_accuracies_6_layers.txt", "w") as output:
...
def main():
    ...
    #TODO!
    parser.add_argument('-train_image_feat', type=str, required=True)
    parser.add_argument('-val_image_feat', type=str, required=True)
    ...
    train_image_feat = np.load(args.train_image_feat)
    val_image_feat = np.load(args.val_image_feat)
    training_data, validation_data = prepare_dataloaders(data, train_image_feat, val_image_feat, opt)
...
def prepare_dataloaders(data, train_image_feat, val_image_feat, opt):
    # ========= Preparing DataLoader =========#
    train_loader = torch.utils.data.DataLoader(
        TranslationDataset(
            src_word2idx=data['dict']['src'],
            tgt_word2idx=data['dict']['tgt'],
            image_features=train_image_feat,
